...
class PickUpAction(ActionDescription):
    ...
    def validate(self, result: Optional[Any] = None, max_wait_time: Optional[timedelta] = None):
        ...
        if not has_gripper_grasped_body(self.arm, self.object_designator):
            if self.object_designator in World.robot.attachments:
                World.robot.detach(self.object_designator)
            raise ObjectNotGraspedError(self.object_designator, World.robot, self.arm, grasp=self.grasp_description)
